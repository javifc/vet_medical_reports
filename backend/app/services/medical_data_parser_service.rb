...
# Reescritura robusta orientada a OCR y multilenguaje.
# Cambios principales:
# - Normalización del texto (normaliza espacios, elimina returns CR, reemplaza NBSP, normaliza UTF-8)
# - Uso de `String#[]` con regex para extraer grupo 1 (más fiable que match + captures.first)
# - Uso de patterns más tolerantes (aceptan acentos, apóstrofes, guiones y múltiples palabras)
# - Extracción multi-línea con lazy quantifiers y delimitador de siguiente label
# - `reject { |_,v| v.nil? || v.to_s.strip.empty? }` en vez de compact
# - Logging claro y manejable
# - Métodos auxiliares para crear patrones finales y para probar texto
...
class MedicalDataParserService
    ...
    def parse
      ...
      if groq_data.size >= DEFAULT_MIN_FIELDS_FOR_GROQ
        ...
        return compact_hash(groq_data)
      else
        Rails.logger.info("PARSER - Groq data insufficient (#{groq_data.size} < #{DEFAULT_MIN_FIELDS_FOR_GROQ}), falling back to rules")
      end
    else
